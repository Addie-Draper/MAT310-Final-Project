...
if __name__ == "__main__":
    cleaned_path = "data/processed/train(1)_cleaned.csv"
    df = load_dataset(cleaned_path)

    train, val, test = split_dataset(df)

    os.makedirs("data/processed", exist_ok=True)
    train.to_csv("data/processed/train(1)_train.csv", index=False)
    val.to_csv("data/processed/train(1)_validation.csv", index=False)
    test.to_csv("data/processed/train(1)_test.csv", index=False)
